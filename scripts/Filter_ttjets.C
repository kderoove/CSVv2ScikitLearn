...
// Example of Root macro based on $ROOTSYS/tutorials/tree/copytree3.C
  ...
  gSystem->Load("$ROOTSYS/test/libEvent");
    ...
    string inputname = "/user/kderoove/bTagging/MoveCSVv2ToMVApackage/CMSSW_7_4_14/src/RecoBTag/TagVarExtractor/test/JetTaggingVariables_ttjetsMLM_76X.root";
    string treename  = "tagVars/ttree";
//    string inputname = filename;
//    string treename  = Treename;
    bool debug = false;

	Int_t max_nb_ofjets	=9999999;
    ...
    cout << "There are " << nentries << " jets in the file " << inputname << " will select maximally " << max_nb_ofjets << " in each pt/eta bin per flavour and vtx category" << endl;
  	...
  	Float_t trackPtRel_0,trackPtRatio_1;
  	...
  	oldtree->SetBranchAddress("TagVarCSV_trackPtRatio_1",&trackPtRatio_1);
	...
	TFile *newfile_VertexCatMinusOne = new TFile("../FlatTrees_tt/VertexCatMinusOne.root","RECREATE");
  	TTree *newtree_VertexCatMinusOne = oldtree->CloneTree(0);
	TFile *newfile_PU = new TFile("../FlatTrees_tt/PU.root","RECREATE");
  	TTree *newtree_PU = oldtree->CloneTree(0);

	
	TFile *newfile_RecoVertex_B = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVRecoVertex_B.root","RECREATE");
  	TTree *newtree_RecoVertex_B = oldtree->CloneTree(0);
  	newtree_B.push_back(newtree_RecoVertex_B);
	TFile *newfile_RecoVertex_C = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVRecoVertex_C.root","RECREATE");
  	TTree *newtree_RecoVertex_C = oldtree->CloneTree(0);
  	newtree_C.push_back(newtree_RecoVertex_C);
	TFile *newfile_RecoVertex_DUSG = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVRecoVertex_DUSG.root","RECREATE");
  	TTree *newtree_RecoVertex_DUSG = oldtree->CloneTree(0);
  	newtree_DUSG.push_back(newtree_RecoVertex_DUSG);
	TFile *newfile_PseudoVertex_B = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVPseudoVertex_B.root","RECREATE");
  	TTree *newtree_PseudoVertex_B = oldtree->CloneTree(0);
  	newtree_B.push_back(newtree_PseudoVertex_B);
	TFile *newfile_PseudoVertex_C = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVPseudoVertex_C.root","RECREATE");
  	TTree *newtree_PseudoVertex_C = oldtree->CloneTree(0);
  	newtree_C.push_back(newtree_PseudoVertex_C);
	TFile *newfile_PseudoVertex_DUSG = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVPseudoVertex_DUSG.root","RECREATE");
  	TTree *newtree_PseudoVertex_DUSG = oldtree->CloneTree(0);
  	newtree_DUSG.push_back(newtree_PseudoVertex_DUSG);
	TFile *newfile_NoVertex_B = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVNoVertex_B.root","RECREATE");
  	TTree *newtree_NoVertex_B = oldtree->CloneTree(0);
  	newtree_B.push_back(newtree_NoVertex_B);
	TFile *newfile_NoVertex_C = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVNoVertex_C.root","RECREATE");
  	TTree *newtree_NoVertex_C = oldtree->CloneTree(0);
  	newtree_C.push_back(newtree_NoVertex_C);
	TFile *newfile_NoVertex_DUSG = new TFile("../FlatTrees_tt/skimmed_20k_eachptetabin_CombinedSVNoVertex_DUSG.root","RECREATE");
	...
	for (Int_t i=0;i<nentries; i++)
	{
        ...
        if(trackPtRel_0 != trackPtRel_0) continue;
        if(jetNTracks<2) continue;
        if(trackPtRatio_1 != trackPtRatio_1) continue;
